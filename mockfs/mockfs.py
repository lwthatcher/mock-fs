...
class MockFileSystem:
  ...
  def Create(self, _type, name, path):
    ...
    if not path and _type != 'drive':
      raise IllegalFSOpError('Only drives can be top-level entities.')
  ...
  def _child_paths(self, full_path):
    """returns list of paths of all children, as well as the provided path"""
    def childof(a, b):
      return a[:len(b)] == b
    result = [k for k in self._entities.keys() if childof(k, full_path)]
    return result
  # endregion
